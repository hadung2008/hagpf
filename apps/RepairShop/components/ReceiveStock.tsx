...
import { PlusIcon, TrashIcon, ChevronDownIcon, ChevronUpIcon } from '../../../components/icons';
...
export const ReceiveStock: React.FC<ReceiveStockProps> = ({ products, onReceiveStock, stockReceiveLogs, allUsers, categories }) => {
    ...
    const handleAddItem = () => {
        if (!selectedProductId || quantity <= 0) {
            alert(t('repairShop.inventory.alertSelectProduct'));
            return;
        }
    ...
    const handleReceiveAll = () => {
        if (itemsToReceive.length === 0) {
            alert(t('repairShop.inventory.alertAddItems'));
            return;
        }
    ...
    return (
        <div className="space-y-8">
            ...
            <div>
                ...
                <div className="bg-gray-50 dark:bg-slate-900/50 p-6 rounded-lg border border-gray-200 dark:border-slate-700 space-y-6">
                    {/* Item add form */}
                            ...
                            <div className="space-y-2">
                                {itemsToReceive.map(item => (
                                    <div key={item.productId} className="flex items-center justify-between bg-white dark:bg-slate-700/50 p-2 rounded-md">
                                        <span className="text-gray-900 dark:text-slate-200">{item.productName}</span>
                                        <div className="flex items-center">
                                            <span className="font-bold mr-4 text-gray-800 dark:text-slate-300">{t('repairShop.inventory.qty', { qty: item.quantity })}</span>
            ...
            <div>
                 ...
                 <div className="bg-white dark:bg-slate-900/50 rounded-lg border border-gray-200 dark:border-slate-700">
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                           <thead>
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">{t('repairShop.inventory.supplierHeader')}</th>
                                ...
                                {stockReceiveLogs.map(log => (
                                    <React.Fragment key={log.id}>
                                        <tr className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                            <td className="px-4 py-3 text-sm text-gray-700 dark:text-slate-300">{new Date(log.date).toLocaleString()}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700 dark:text-slate-300">{getUserName(log.userId)}</td>
                                            <td className="px-4 py-3 text-sm text-gray-500 dark:text-slate-400">{log.supplier || t('general.na')}</td>
