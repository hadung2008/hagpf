...
const ReportCard: React.FC<ReportCardProps> = ({ title, description, icon, onClick }) => (
    <div 
        onClick={onClick}
        className={`bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-4 sm:p-6 hover:bg-gray-50 dark:hover:bg-slate-700/50 hover:border-indigo-500 transition-all ${onClick ? 'cursor-pointer' : ''}`}
    >
        <div className="flex items-start gap-4">
            <div className="bg-gray-100 dark:bg-slate-900 p-3 rounded-lg flex-shrink-0">
                {icon}
            </div>
            <div>
    ...
    const reports = [
        {
            ...
            icon: <CurrencyDollarIcon className="w-6 h-6 sm:w-8 sm:h-8 text-green-500 dark:text-green-400" />
        },
        {
            id: 'best-selling',
            title: t('repairShop.reports.bestSellingTitle'),
            description: t('repairShop.reports.bestSellingDesc'),
            icon: <TagIcon className="w-6 h-6 sm:w-8 sm:h-8 text-indigo-500 dark:text-indigo-400" />
        },
        {
            id: 'repair-volume',
            title: t('repairShop.reports.repairVolumeTitle'),
            description: t('repairShop.reports.repairVolumeDesc'),
            icon: <TicketIcon className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-500 dark:text-yellow-400" />
        },
        {
            id: 'sales-employee',
            title: t('repairShop.reports.salesEmployeeTitle'),
            description: t('repairShop.reports.salesEmployeeDesc'),
            icon: <UserGroupIcon className="w-6 h-6 sm:w-8 sm:h-8 text-blue-500 dark:text-blue-400" />
        },
        {
            id: 'inventory',
            title: t('repairShop.reports.inventoryTitle'),
            description: t('repairShop.reports.inventoryDesc'),
            icon: <ServerStackIcon className="w-6 h-6 sm:w-8 sm:h-8 text-purple-500 dark:text-purple-400" />
        },
        {
            id: 'export-data',
            title: t('repairShop.reports.exportDataTitle'),
            description: t('repairShop.reports.exportDataDesc'),
            icon: <ArrowDownTrayIcon className="w-6 h-6 sm:w-8 sm:h-8 text-gray-500 dark:text-slate-400" />
        },
    ];
