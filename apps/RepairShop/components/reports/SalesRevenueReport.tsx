        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('repairShop.reports.salesRevenueTitle')}</h1>
                </div>
                <div className="bg-gray-200 dark:bg-slate-700 rounded-lg p-1 flex">
                    {timeFilters.map(filter => (
                        <button key={filter.id} onClick={() => setTimeFilter(filter.id)} className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${timeFilter === filter.id ? 'bg-indigo-600 text-white' : 'text-gray-700 dark:text-slate-300 hover:bg-gray-300 dark:hover:bg-slate-600/50'}`}>
                            {filter.label}
                        </button>
                    ))}
                </div>
