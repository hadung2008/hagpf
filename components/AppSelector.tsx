...
const AppCard: React.FC<AppCardProps> = ({ app, isPinned, isAdmin, onSelectApp, onTogglePin, onOpenEditModal, onOpenDeleteConfirm }) => {
...
export const AppSelector: React.FC<AppSelectorProps> = ({ currentUser, onSelectApp, onLogout, theme, setTheme }) => {
  ...
  const apps: AppInfo[] = useMemo(() => appsData
    .filter(app => app.id !== 'RolePermissions' || isAdmin)
    .map(app => {
