...
import { RectangleGroupIcon, ShieldCheckIcon, ArrowLeftOnRectangleIcon, PinIcon, PencilIcon, TrashIcon, PlusIcon, iconMap } from './icons';
...
const initialApps: Omit<AppInfo, 'name' | 'description'>[] = [
  { 
    id: 'ApprovalFlow', 
...
export const AppSelector: React.FC<AppSelectorProps> = ({ currentUser, onSelectApp, onLogout, theme, setTheme }) => {
  const { t } = useTranslation();
  const [appsData, setAppsData] = useState(initialApps);
  ...
  const handleSaveApp = (appData: Omit<AppInfo, 'lastModified' | 'name' | 'description'>) => {
    // Note: Saving new apps won't have translations until they are added to locale files.
    // This example handles saving the structure, but not the dynamic translation generation.
            ...
            <div className="flex justify-between items-center h-16">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <button
                        ...
                        className="flex items-center justify-center h-10 w-10 rounded-full text-gray-500 dark:text-slate-400 bg-gray-200 dark:bg-slate-800 hover:bg-gray-300 dark:hover:bg-slate-700 hover:text-gray-800 dark:hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-gray-100 dark:focus:ring-offset-slate-900 transition-colors"
                    >
